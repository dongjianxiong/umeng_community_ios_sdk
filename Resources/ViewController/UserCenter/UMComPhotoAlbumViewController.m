...
- (NSArray *)imageModelsWithData:(NSArray *)data
{
    ...
    for (UMComAlbum *album in data) {
        NSArray *subArr = [album imageModels];
        if (subArr.count > 0) {
            [imageModels addObjectsFromArray:subArr];
        }
    }
    return imageModels;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < self.imageModelsArray.count) {
        UMComImageModel *imageModel = self.imageModelsArray[indexPath.row];
        [cell.imageView setImageURL:imageModel.smallImageUrlString placeHolderImage:[UIImage imageNamed:@"image-placeholder"]];
    }else{
